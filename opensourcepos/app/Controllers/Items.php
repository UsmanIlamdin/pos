...
class Items extends Secure_Controller
{
    ...
    private function validateItemFields(int $itemId): ?ResponseInterface
    {
        ...
        foreach ($taxNamesInput as $index => $taxName) {
            $taxName = trim((string) $taxName);
            $taxPercent = trim((string) ($taxPercentsInput[$index] ?? ''));

            if ($taxName === '' && $taxPercent === '') {
                continue;
            }

            $rules["tax_names.$index"] = 'required|max_length[255]';

            $messages["tax_names.$index"] = [
